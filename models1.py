...
class SimpleRNN(nn.Module):
    ...
    def __call__(self, carry, x):
        rnn = RNNCell()
        print(carry.shape, x.shape)
        params = rnn.init(self.make_rng('params'), carry, xs)
        new_carry, new_out = rnn.apply(params, carry, xs)

        # new_carry, new_out = rnn(carry, x)
        # J = jacfwd(rnn, has_aux=False)(carry, x)
        return new_carry, (new_out, jnp.zeros_like(carry.shape[-1], carry.shape[-1]))
...
class SimpleRNNClassifier(nn.Module):
    ...
    @nn.compact
    def __call__(self, carry, x):
        SimpleRNNScan = nn.scan(SimpleRNN, variable_broadcast='params', split_rngs={'params': False}, in_axes=1, out_axes=1)
        rnn_state, (y, J) = SimpleRNNScan()(carry, x)
